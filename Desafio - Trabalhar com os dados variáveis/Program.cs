...
do
{
    ...
    switch (menuSelection)
    {
        ...
        case "2":
            ...
            string[] multCharacteristics = new string[4];

            while (dogCharacteristic == "")
            {
                // #2 have user enter multiple comma separated characteristics to search for
                Console.WriteLine($"\r\nEnter desired dog characteristic to search for, separated for \",\", max 4 characteristics");
                readResult = Console.ReadLine()?.Trim(' ').ToLower();

                multCharacteristics = readResult!.Split(",");
                Array.Sort(multCharacteristics);

                if (multCharacteristics != null)
                {
                    foreach (string characteristic in multCharacteristics)
                        dogCharacteristic = characteristic;
                    Console.WriteLine();
                }
            }
            ...
            for (int i = 0; i < maxPets; i++)
            {
                ...
                if (ourAnimals[i, 1].Contains("dog"))
                {
                    ...
                    foreach (string searchCharacteristic in multCharacteristics!)
                    {
                        for (int j = 2; j > -1; j--)
                        {
                            ...
                            {
                                Console.Write($"\rsearching our dog {ourAnimals[i, 3]} for {searchCharacteristic.Trim()} {icon}");
                                Thread.Sleep(20);
                            }
                    ...
                    foreach (string searchCharacteristic in multCharacteristics!)
                    {
                        if (dogDescription.Contains(searchCharacteristic))
                        {
                            // #3b update message to reflect term 

                            // #3c set a flag "this dog" is a match
                            Console.Write($"\nOur dog {ourAnimals[i, 3]} is a match for: {searchCharacteristic}!");
